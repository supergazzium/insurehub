...
<template>
  <div class="max-w-5xl mx-auto">
    <div class="flex flex-col">
      <header class="py-2 mb-4 flex items-center justify-between gap-3 flex-wrap">
        <div v-if="customer">
        ...
        <button class="px-3 py-1.5 rounded-lg text-sm text-slate-600 hover:bg-slate-100 flex items-center gap-1.5" @click="goBack">
          <i class="pi pi-arrow-left text-xs" /> กลับ
        </button>
      </header>

      <div v-if="errorMsg" class="mb-4 p-4 bg-rose-50 border border-rose-200 rounded-lg text-rose-700 text-sm">
        {{ errorMsg }}
      </div>

      <div v-if="customer" class="space-y-6 pb-8">
        <!-- Identity -->
        <section>
