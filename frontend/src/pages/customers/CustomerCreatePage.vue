...
const form = reactive({
  ...
  mailingSubDistrict: '',
  mailingPostcode: '',
  // ผู้ติดต่อแทน (contact person) — all types
  contactName: '',
  contactPosition: '',
  contactPhone: '',
  contactEmail: '',
...
// Live check on the active identity number as the operator types:
//   individual        → เลขบัตรประชาชน (id_card)
//   foreign_individual → หนังสือเดินทาง (passport)
//   corporate          → เลขนิติบุคคล / ผู้เสียภาษี (tax_id)
// Queries GET /customers?q=<value> (already matches id_card/tax_id/passport)
// and confirms an EXACT match on the right field. Backend still enforces the
// unique constraint at submit; this surfaces the existing customer first so
// the operator can open them instead of creating a duplicate.
const dupMatch = ref<CustomerListRow | null>(null)
const dupChecking = ref(false)
let dupTimer: number | undefined

/** Which field + value drives the dup check for the current type. */
const dupField = computed<{ field: 'idCard' | 'passport' | 'taxId'; value: string } | null>(() => {
  if (form.customerType === 'individual') {
    const v = form.idCard.trim()
    return /^\d{13}$/.test(v) ? { field: 'idCard', value: v } : null
  }
  if (form.customerType === 'foreign_individual') {
    const v = form.passport.trim().toUpperCase()
    return isPassportNumber(v) ? { field: 'passport', value: v } : null
  }
  const v = form.taxId.trim()
  return /^\d{13}$/.test(v) ? { field: 'taxId', value: v } : null
})

async function runDupCheck(): Promise<void> {
  const target = dupField.value
  if (!target) { dupMatch.value = null; dupChecking.value = false; return }
  dupChecking.value = true
  try {
    const res = await fetchCustomerList({ q: target.value, perPage: 10 })
    // Exact match on the SAME identity field (q also matches phone/email, so
    // filter to the field we actually care about). passport compared upper.
    const hit = res.data.find((r) => {
      if (target.field === 'idCard') return (r.idCard ?? '').trim() === target.value
      if (target.field === 'taxId') return (r.taxId ?? '').trim() === target.value
      return (r.passport ?? '').trim().toUpperCase() === target.value
    })
    dupMatch.value = hit ?? null
  } catch {
    dupMatch.value = null
  } finally {
...
// Debounced re-check whenever the driving identity value changes.
watch(dupField, () => {
  dupMatch.value = null
  if (dupTimer) window.clearTimeout(dupTimer)
  if (!dupField.value) return
  dupChecking.value = true
  dupTimer = window.setTimeout(() => void runDupCheck(), 350)
})

/** Human label for the matched duplicate (name + code). */
const dupLabel = computed(() => {
  const r = dupMatch.value
  if (!r) return ''
  const name = r.customerType === 'corporate'
    ? (r.juristicName || '(นิติบุคคล)')
    : [r.firstName, r.lastName].filter(Boolean).join(' ') || '(ลูกค้า)'
...
const errContactEmail = computed(() => validateEmailFmt(form.contactEmail))
...
type FieldName =
  ...
  | 'phone' | 'telPhone' | 'email' | 'email2' | 'contactEmail' | 'birthDate'
const touched = reactive<Record<FieldName, boolean>>({
  ...
  phone: false, telPhone: false, email: false, email2: false, contactEmail: false, birthDate: false,
})
...
const showContactEmail = computed(() => displayError('contactEmail', errContactEmail.value))
...
const canSubmit = computed(() => {
  ...
  if (errContactEmail.value !== null) return false
  if (errBirth.value !== null) return false
  // Block while an identity number matches an existing customer.
  if (dupMatch.value !== null) return false
  return true
})
...
function collectValidationProblems(): string[] {
  ...
  push('อีเมลผู้ติดต่อแทน', errContactEmail.value)
  push('วันเกิด', errBirth.value)
  if (dupMatch.value !== null) {
    const idLabel = form.customerType === 'corporate'
      ? 'เลขนิติบุคคล/ผู้เสียภาษี'
      : form.customerType === 'foreign_individual' ? 'หนังสือเดินทาง' : 'เลขบัตรประชาชน'
    problems.push(`${idLabel}: มีลูกค้ารายนี้อยู่แล้ว (${dupLabel.value})`)
  }
  return problems
}
...
const payload = computed(() => {
  ...
  // ผู้ติดต่อแทน — send only when at least one field is filled; backend
  // maps contactPerson.{name,phone,email,position} → contact_* columns.
  if (form.contactName.trim() || form.contactPhone.trim() || form.contactEmail.trim() || form.contactPosition.trim()) {
    base.contactPerson = {
  ...
  <div class="space-y-6 max-w-5xl">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <template v-if="isPerson">
          ...
          <FormField v-if="form.customerType === 'individual'"
            ...
            <p v-if="dupChecking" class="text-[10px] text-slate-400 mt-0.5"><i class="pi pi-spin pi-spinner mr-1" />กำลังตรวจสอบข้อมูลซ้ำ…</p>
            <div v-else-if="dupMatch" class="mt-1 flex items-center gap-2 rounded-md bg-amber-50 border border-amber-300 px-2 py-1.5 text-xs text-amber-800">
              <i class="pi pi-exclamation-triangle" />
              <span class="flex-1">เลขบัตรประชาชนนี้มีลูกค้าอยู่แล้ว: <span class="font-medium">{{ dupLabel }}</span></span>
              <button type="button" @click="openDuplicate" class="underline hover:no-underline whitespace-nowrap">เปิดดู</button>
            </div>
          </FormField>
          ...
          <FormField v-if="form.customerType === 'foreign_individual'"
            ...
            <div v-else-if="dupMatch" class="mt-1 flex items-center gap-2 rounded-md bg-amber-50 border border-amber-300 px-2 py-1.5 text-xs text-amber-800">
              <i class="pi pi-exclamation-triangle" />
              <span class="flex-1">หนังสือเดินทางนี้มีลูกค้าอยู่แล้ว: <span class="font-medium">{{ dupLabel }}</span></span>
              <button type="button" @click="openDuplicate" class="underline hover:no-underline whitespace-nowrap">เปิดดู</button>
            </div>
          </FormField>
        ...
        <template v-if="form.customerType === 'corporate'">
          ...
          <FormField label="เลขประจำตัวผู้เสียภาษี" class="col-span-2" error-key="taxId" :errors="fieldErrors">
            ...
            <div v-else-if="dupMatch" class="mt-1 flex items-center gap-2 rounded-md bg-amber-50 border border-amber-300 px-2 py-1.5 text-xs text-amber-800">
              <i class="pi pi-exclamation-triangle" />
              <span class="flex-1">เลขนิติบุคคล/ผู้เสียภาษีนี้มีลูกค้าอยู่แล้ว: <span class="font-medium">{{ dupLabel }}</span></span>
              <button type="button" @click="openDuplicate" class="underline hover:no-underline whitespace-nowrap">เปิดดู</button>
            </div>
          </FormField>
        </template>
    ...
    <section class="card p-5">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <FormField label="ตำแหน่ง" error-key="contactPerson.position" :errors="fieldErrors">
          <input v-model.trim="form.contactPosition" placeholder="เช่น ผู้จัดการ, เจ้าของกิจการ"
            class="w-full border border-slate-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-brand-400" />
        </FormField>
        <FormField label="เบอร์โทรผู้ติดต่อแทน" error-key="contactPerson.phone" :errors="fieldErrors">
          <input v-model.trim="form.contactPhone" inputmode="tel" placeholder="0812345678"
            class="w-full border border-slate-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-brand-400" />
        </FormField>
        <FormField label="อีเมลผู้ติดต่อแทน" error-key="contactPerson.email" :errors="fieldErrors">
          <input v-model.trim="form.contactEmail" type="email" placeholder="contact@example.com"
            :class="inputClass(showContactEmail, false)"
